...
| `bloques_a_actualizar` | Bloques actualizados | Bloques protegidos (copia exacta del histórico) |
|---|---|---|
...
**Quién decide:** el usuario, normalmente a través de checkboxes en la UI (ver
`streamlit_app.py`, sidebar "Actualización parcial"). Verificado end-to-end: probado con
Groq real protegiendo `evento`/`cliente`/`ponentes` mientras se actualizaba solo `nota_bene`.
...
- **Estructura del histórico** (`src/schemas.py::crear_estructura_vacia_historico`), igual
  ...
  ```
- **Cómo fusiona el LLM:** recibe el histórico completo dentro del prompt de sistema (ver
  `src/llm.py::construir_prompt_sistema`) con instrucciones explícitas de mantener lo que ya
  existía y solo actualizar lo que el nuevo texto modifique. Los cambios de presupuesto se
  destacan así: `"3200€ (anterior: 2500€)"`.
- **`contexto.modo_actualizacion`:** `"fusionar"` (por defecto) o `"sobrescribir"`.
- `streamlit_app.py` sigue incluyendo un histórico **local, solo de sesión** (por
  `id_evento`), útil para probar el flujo sin conexión a la BD real; si la BD SÍ está
  conectada, tiene prioridad el histórico local si lo activas explícitamente, si no, se usa
  el de la BD automáticamente.
...
├── streamlit_app.py           ← interfaz de prueba (subida de archivo/texto, id_evento,
│                                  bloques a actualizar, histórico local por evento)
...
│   ├── nucleo.py                ← valida entrada, llama a src/llm.py, gestiona histórico
│   │                                (explícito o autocargado de la BD, ver sección 8)
...
│   │                                motor único "llm"...)
│   ├── llm.py                   ← único motor: Groq, prompt + histórico + bloques parciales
...
```text
1. Quien llame (main.py, streamlit_app.py, o el futuro backend) construye el payload y
...
4. src/llm.py construye el prompt de sistema (prompts/prompt_sistema.md + esquema +,
   si aplica, instrucciones de histórico y/o de protección de bloques) y llama a Groq
   con temperature=0 y salida JSON forzada.
...
6. src/schemas.py::generar_aviso_y_validacion calcula el % de campos obligatorios del
   bloque Evento y el mensaje de aviso.
7. Si había historial_anterior, src/nucleo.py añade una entrada en
   nota_bene.informacion_adicional.historico_actualizaciones y actualiza
   nota_bene.cabecera.ultima_actualizacion.
8. src/schemas.py::construir_salida_base arma la respuesta final (contrato común).
```
...
`id_evento` para probar el modo actualización, y las 4 pestañas de resultado (Evento,
Cliente, Ponentes, Nota Bene) más el JSON completo.
...
- [ ] **Pendiente, sigue sin definir:** cómo invoca el backend a `ejecutar_agente(payload)` en sí (API REST / librería / otro) — la conexión a BD resuelve la lectura de estado, no esta decisión (ver sección 8.2).
